...
;
    ...
    /**
     * Determines which alias to use for Doctrine ORM Mapping attributes.
     *
     * This function decides which alias should be used when adding a Doctrine ORM attribute.
     * Current implementation uses "ORM" as the default fallback.
     *
     * Future enhancements could include:
     * - Analyzing project conventions
     * - User preference settings
     * - Smart detection based on existing code patterns
     *
     * @param scopeForUseOperator The PHP scope where the use statement should be added
     * @return The alias to use (e.g., "ORM", "Doctrine", etc.)
     */
    private static String determineDoctrineOrmAlias(@NotNull PhpPsiElement scopeForUseOperator) {
        // TODO: Future enhancement - could analyze existing imports or ask user preference
        // For now, always fallback to "ORM" as the default alias
        return "ORM";
    }

    /**
     * Insert handler that adds a Doctrine ORM attribute with proper alias handling
     */
    private record PhpDoctrineAttributeInsertHandler(@NotNull String attributeFqn, @NotNull String shortClassName) implements InsertHandler<LookupElement> {
        ...
        public void handleInsert(@NotNull InsertionContext context, @NotNull LookupElement item) {
            ...
            if (scopeForUseOperator != null) {
                // Check if there's already an alias for Doctrine\ORM\Mapping
                Map<String, String> aliasesInScope = PhpCodeInsightUtil.getAliasesInScope(scopeForUseOperator);
                boolean alreadyImported = false;
                String existingAlias = null;

                for (Map.Entry<String, String> entry : aliasesInScope.entrySet()) {
                    if (DOCTRINE_MAPPING_NAMESPACE.equals(entry.getValue())) {
                        existingAlias = entry.getKey();
                ...
                if (alreadyImported) {
                    // Use existing alias
                    importedAlias = existingAlias;
                } else {
                    // Determine which alias to use via helper function
                    importedAlias = determineDoctrineOrmAlias(scopeForUseOperator);

                    // Insert "use Doctrine\ORM\Mapping as [alias];" manually
                    PsiFile containingFile = scopeForUseOperator.getContainingFile();
                    if (containingFile instanceof PhpFile phpFile) {
                        // Find insertion point (after namespace or after last use statement)
                        PsiElement anchor = null;

                        // Look for existing use statements in top level defs
                        for (PhpNamedElement def : phpFile.getTopLevelDefs().values()) {
                            if (def instanceof PhpUseList) {
                                anchor = def;
                            } else if (def instanceof PhpNamespace && anchor == null) {
                                anchor = def;
                            }
                        }

                        if (anchor != null) {
                            // Create the use statement with determined alias
                            String useStatementText = "use " + DOCTRINE_MAPPING_NAMESPACE.substring(1) + " as " + importedAlias + ";\n";
                            int insertOffset = anchor.getTextRange().getEndOffset();

                            // Find the end of line after anchor
                            CharSequence fileText = document.getCharsSequence();
                            while (insertOffset < fileText.length() && fileText.charAt(insertOffset) != '\n') {
                                insertOffset++;
                            }
                            if (insertOffset < fileText.length()) {
                                insertOffset++; // Move past the newline
                            }

                            document.insertString(insertOffset, useStatementText);
                            PsiDocumentManager.getInstance(project).commitDocument(document);
                        }
                    }
                }
            } else {
                // Fallback if we can't find scope
                importedAlias = "ORM";
            }
            ...
            // Build attribute text using the alias (e.g., "#[ORM\Column]")
            String newline = hasNewlineAfter ? "" : "\n";
            String attributeText = "#[" + importedAlias + "\\" + shortClassName + "]" + newline;
