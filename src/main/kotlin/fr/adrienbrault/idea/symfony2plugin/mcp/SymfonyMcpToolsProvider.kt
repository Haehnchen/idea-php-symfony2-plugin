...
import fr.adrienbrault.idea.symfony2plugin.mcp.toolset.SymfonyToolset
...
class SymfonyMcpToolsProvider : McpToolsProvider {
    ...
    override fun getTools(): List<McpTool> {
        ...
        return try {
            SymfonyToolset().asTools()
        } catch (e: Exception) {
            println("[Symfony MCP] Error loading tools from SymfonyToolset: ${e.message}")
            emptyList()
        }
    }
}
