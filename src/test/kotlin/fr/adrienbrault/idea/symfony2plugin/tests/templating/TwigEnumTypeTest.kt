...
class TwigEnumTypeTest : SymfonyLightCodeInsightFixtureTestCase() {
    override fun setUp() {
        super.setUp()
    ...
    fun testEnumFunctionCompletesCases() {
    ...
    fun testEnumCaseNavigationInsideFunctionArgument() {
        assertNavigationMatch(TwigFileType.INSTANCE,
            "{{ render_status(enum('Example\\\\Status').Ac<caret>tive, format: 'text') }}",
            PlatformPatterns.psiElement(PhpEnumCase::class.java).withName("Active"))
    }

    fun testEnumCaseRetainsItsTypeInMemberChains() {
        ...
        assertNavigationMatch(TwigFileType.INSTANCE,
            ...
            PlatformPatterns.psiElement(Method::class.java).withName("description"))
    }

    fun testEnumCasesAreInspected() {
        assertLocalInspectionNotContains(TwigVariablePathInspection::class.java, "f.html.twig",
            "{{ render_status(enum('Example\\\\Status').Ac<caret>tive, format: 'text') }}",
            "Field or method not found")
        assertLocalInspectionContains(TwigVariablePathInspection::class.java, "f.html.twig", "{{ enum('Example\\\\Status').Un<caret>known }}", "Field or method not found")
        assertLocalInspectionContains(TwigVariablePathInspection::class.java, "f.html.twig", "{{ enum('Example\\\\Status').ac<caret>tive }}", "Field or method not found")
    }

    fun testInvalidOrDynamicArgumentsDoNotResolveAsEnums() {
