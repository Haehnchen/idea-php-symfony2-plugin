...
class SymfonyVarDirectoryWatchScopeUtilTest : TestCase() {
    ...
    fun testIsCachePath() {
        assertTrue(SymfonyVarDirectoryWatchScopeUtil.isCachePath("/project/var/cache/dev/appDevUrlGenerator.php"))
        assertTrue(SymfonyVarDirectoryWatchScopeUtil.isCachePath("/project/app/cache/dev/appDevUrlGenerator.php"))
        assertTrue(SymfonyVarDirectoryWatchScopeUtil.isCachePath("/project/var/cache/prod/container.xml"))
        assertFalse(SymfonyVarDirectoryWatchScopeUtil.isCachePath("/project/config/routes.yaml"))
        assertFalse(SymfonyVarDirectoryWatchScopeUtil.isCachePath("/project/src/Controller/FooController.php"))
    }

    fun testIsCompiledRoutePath() {
        // matches
        assertTrue(SymfonyVarDirectoryWatchScopeUtil.isCompiledRoutePath("/project/var/cache/dev/appDevUrlGenerator.php"))
        assertTrue(SymfonyVarDirectoryWatchScopeUtil.isCompiledRoutePath("/project/app/cache/dev/appDevUrlGenerator.php"))
        assertTrue(SymfonyVarDirectoryWatchScopeUtil.isCompiledRoutePath("/project/var/cache/dev/url_generating_routes.php"))
        assertTrue(SymfonyVarDirectoryWatchScopeUtil.isCompiledRoutePath("/project/var/cache/dev/srcDevDebugProjectContainerUrlGenerator.php"))
        assertTrue(SymfonyVarDirectoryWatchScopeUtil.isCompiledRoutePath("/project/var/cache/dev_123/appDevUrlGenerator.php"))

        // rejects (prod/dev distinction is handled by isCachePath pre-filter in the watcher)
        assertFalse(SymfonyVarDirectoryWatchScopeUtil.isCompiledRoutePath("/project/var/cache/dev/translations/catalogue.en.php"))
        assertFalse(SymfonyVarDirectoryWatchScopeUtil.isCompiledRoutePath("/project/var/cache/dev/App_KernelDevDebugContainer.xml"))
        assertFalse(SymfonyVarDirectoryWatchScopeUtil.isCompiledRoutePath("/project/var/cache/dev/some_other_file.php"))
    }

    fun testCollectScopesForPathsMatchesAllDefaultRouteFileNames() {
        val scopes = SymfonyVarDirectoryWatchScopeUtil.createScopes("/project", Settings())

        val files = listOf(
            "/project/var/cache/dev/appDevUrlGenerator.php",
            "/project/var/cache/dev/appDevDebugProjectContainerUrlGenerator.php",
            "/project/var/cache/dev/srcDevDebugProjectContainerUrlGenerator.php",
            "/project/var/cache/dev/url_generating_routes.php",
            "/project/var/cache/dev/UrlGenerator.php",
            "/project/app/cache/dev/appDevUrlGenerator.php",
        )

        for (file in files) {
            val matched = SymfonyVarDirectoryWatchScopeUtil.collectScopesForPaths(scopes, listOf(file))
            assertTrue("Expected ROUTES scope for $file", matched.contains(SymfonyVarDirectoryWatcher.Scope.ROUTES))
        }
    }
