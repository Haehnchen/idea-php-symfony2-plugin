...
public class PhpAttributeCompletionContributorTest extends SymfonyLightCodeInsightFixtureTestCase {
    ...
    public void testMethodLevelAttributeInsertion() {
        String cacheResult = insertSelectedCompletion(
        ...
        assertTrue("Result should contain use statement", filterResult.contains("use Twig\\Attribute\\AsTwigFilter;"));
        assertTrue("Result should contain quotes for filter name", filterResult.contains("#[AsTwigFilter(\"\")]"));

        String functionResult = insertSelectedCompletion(
            "<?php\n\nnamespace App\\Twig;\n\nuse Twig\\Extension\\AbstractExtension;\n\nclass MyExtension extends AbstractExtension {\n    #<caret>\n    public function myFunction() { }\n}",
            "#[AsTwigFunction]"
        );
        assertTrue("Result should contain use statement", functionResult.contains("use Twig\\Attribute\\AsTwigFunction;"));
        assertTrue("Result should contain quotes for function name", functionResult.contains("#[AsTwigFunction(\"\")]"));
    }
    ...
    public void testClassLevelAttributeInsertion() {
        String routeResult = insertSelectedCompletion(
            "<?php\n\nnamespace App\\Controller;\n\n#<caret>\nclass TestController {\n    public function index() { }\n}",
            "#[Route]"
        );
        assertTrue("Result should contain Route use statement", routeResult.contains("use Symfony\\Component\\Routing\\Attribute\\Route;"));
        assertTrue("Result should contain quotes for route path", routeResult.contains("#[Route(\"\")]"));
        assertTrue("Result should have Route attribute before class", routeResult.indexOf("#[Route") < routeResult.indexOf("class TestController"));

        String asControllerResult = insertSelectedCompletion(
            "<?php\n\nnamespace App\\Controller;\n\n#<caret>\nclass TestController {\n    public function index() { }\n}",
            "#[AsController]"
        );
        assertTrue("Result should contain AsController use statement", asControllerResult.contains("use Symfony\\Component\\HttpKernel\\Attribute\\AsController;"));
    ...
    private String insertSelectedCompletion(String content, String lookupString) {
        ...
        var items = myFixture.getLookupElements();
        var completionItem = java.util.Arrays.stream(items)
            .filter(l -> lookupString.equals(l.getLookupString()))
            .findFirst()
            .orElse(null);

        myFixture.getLookup().setCurrentItem(completionItem);
        myFixture.type('\n');
